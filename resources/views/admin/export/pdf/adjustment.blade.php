<table class="table table-striped">
    <thead>
        <tr>
            <th colspan="5" style="background-color: yellow; text-align:center; font-size: 30px; height: 50px; font-weight:50;"><b>{{__('sidebar.stock_adjs')}}</b></th>
        </tr>
        <tr>
            <th style="width: 100px; text-align:center; height: 60px;">{{__('general.date')}}</th>
            <th style="width: 100px; text-align:center; ">{{__('general.store')}}</th> 
            <th style="width: 100px; text-align:center; ">{{__('report.qty_product')}}</th>
            <th style="width: 150px; text-align:center; ">{{__('adjustment.amount_total')}}</th>
            <th style="width: 150px; text-align:center; ">{{__('adjustment.amount_recovered')}}</th>
        </tr>
        <tr>
    ...
    <tbody>
        ...
        @foreach ($data as $d)
        <tr>
            <td style="height: 40px;"> {{ $d->created_at }}</td>
            <td> {{ $d->store->name ?? '' }} </td> 
