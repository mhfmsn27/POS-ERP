...
<div class="modal fade" id="returnUpdate" tabindex="-1" aria-labelledby="returnUpdateLabel" aria-hidden="true">
      <div class="modal-dialog modal-fullscreen-md-down">
            <form method="POST" id="ppReturnStore" class="modal-content">
                  @csrf
                  <div class="modal-header">
                        <h6 class="modal-title" id="returnUpdateLabel">Update Return Pembelian Produk</h6>
...
<script>
      function format(d) {
            return '<table cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">' +
                  ...
                  '</tr>' +
                  '<tr class="mt-2">' +
                  '<td>Qty Dikembalikan</td>' +
                  '<td>: ' + d.qty_return + '</td>' +
                  '</tr>' +
                  '<tr class="mt-2">' +
                  '<td>Qty Adjustment </td>' +
                  '<td>: ' + d.qty_adjusted + '</td>' +
                  '</tr>' +
                  '<tr class="mt-2">' +
                  '<td>Qty Transfer </td>' +
                  '<td>: ' + d.qty_transfer + '</td>' +
                  '</tr>' +
                  '<tr class="mt-2">' +
                  '<td>Qty Expire </td>' +
                  '<td>: ' + d.qty_expire + '</td>' +
      ...
      $(document).ready(function() {
            var sell_table = $('#sellContent').DataTable({
                  ...
                  ajax: {
                        "url": domain + domainpath + '/mobile/transaction/purchase-detail',
                        "data": function(d) {
                              d.store = $('#store').val();
            ...
            $("body").on("change", "#date_now, #supplier", function() {
                  sell_table.ajax.reload();
            });
            ...
            $("form#ppReturnStore").on("submit", function(e) {
                  spinner.show();
                  e.preventDefault();
                  var formData = new FormData(this);
                  setTimeout(function() {
                        $.ajax({
                              url: domain + domainpath + "/mobile/transaction/store-po-return",
                              type: "POST",
                              data: formData,
      ...
      function getreturnmodal(id) {
            var url = domainpath + "/mobile/transaction/return-po-dom/" + id;
            setTimeout(function() {
                  $.ajax({
                        url: domain + url,
                        type: 'GET',
                        ...
                        success: function(data) {
                              ...
                              dataContent = `
                                          <div class="col-md-12 form-group">
                                                ...
                                                <input type="number" required class="form-control qty_return" name="qty_return" id="qte-` + product.po_id + `" min="0" max="` + product.stock + `" value="0">
