        ...
        <div class="row">
            ...
            <div class="col-2">
                <button
                    ...
                    class="btn btn-success label-btn label-end"
                >
                    {{
                        loader.submit
                            ? "Mohon Tunggu...."
                            : "Simpan Pembaharuan dan Proses Pembayaran"
                    }}
...
export default {
    ...
    methods: {
        ...
        async getFakturs() {
            ...
            try {
                const response = await ApiData.get(
                    `app/transactions/transaction-due?limit=${this.faktur.limit}&ref_no=${this.modal.search}&customer=${this.transaction.customer.id}&status=due&type=&order=asc`
                );
                var data = response.data;
            ...
            if (item_id != null) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ApiData.delete(
                            "app/transactions/sales/faktur/delete-item/" +
                                item_id
                        )
                            .then((response) => {
        ...
        updateTransaction() {
            ...
            this.transaction.subtotal = this.transaction.total_due - totalPay;
        },
            ...
            this.$refs.ValidationForOtherInformation.validate().then(
                (success) => {
                    if (!success) {
                        ...
                        NProgress.set(0.1);
                        ApiData.post(
                            "app/transactions/sales/faktur/update/" +
