...
export default {
    ...
    methods: {
        async getOptions() {
            try {
                const response = await ApiData.get(
                    `app/settings/table-view?table=sale_faktur`
                );
        ...
        async getData(page = 1) {
            ...
            try {
                const response = await ApiData.get(
                    `app/transactions/sales?limit=${this.limit}&page=${this.page}&ref=${this.filter.name}&start_date=${startdate}&end_date=${enddate}&createdby=${this.filter.user}&customer=${this.filter.customer}&payment=${this.filter.status}&sort=${this.sort.field}&sortby=${this.sort.order}`
                );
                var data = response.data;
        ...
        deleteDraft(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    NProgress.start();
                    NProgress.set(0.1);
                    ApiData.delete("app/transactions/sales/delete-draft/" + id)
                        .then((response) => {
