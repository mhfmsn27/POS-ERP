                        ...
                        <div class="row">
                            ...
                            <div class="col">
                                ...
                                <SelectOption
                                    ...
                                    :options="[
                                        {
                                            name: 'Belum Di Gunakan',
                                            value: 'shipping_not_use',
                                        },
                                        {
                                            name: 'Sudah Di Gunakan',
                                            value: 'shipping_used',
                                        },
                                    ]"
                                    filter
                    ...
                    >
                        <template #body="{ data }">
                            <span
                                ...
                                v-if="data.status == 'shipping_not_use'"
...
export default {
    ...
    data() {
        return {
            ...
            filter: {
                name: "",
                status: ["shipping_not_use", "shipping_used"],
                date: {
                    start: "",
                    end: "",
                },
    ...
    methods: {
        async getOptions() {
            try {
                const response = await ApiData.get(
                    `app/settings/table-view?table=shipping`
                );
        ...
        async getData(page = 1) {
            ...
            try {
                const response = await ApiData.get(
                    `app/transactions/sales/shipping?limit=${this.limit}&page=${this.page}&ref=${this.filter.name}&start_date=${startdate}&end_date=${enddate}&createdby=${this.filter.user}&customer=${this.filter.customer}&sort=${this.sort.field}&sortby=${this.sort.order}&status=${this.filter.status}`
                );
                var data = response.data;
        ...
        deleteData(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    NProgress.set(0.1);
                    ApiData.delete(
                        "app/transactions/sales/shipping/delete/" + id
                    )
                        .then((response) => {
