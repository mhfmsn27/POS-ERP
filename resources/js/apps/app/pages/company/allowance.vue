        ...
        <div class="card card-block card-stretch card-height">
            ...
            <Form @submit="ValidationFormData()" ref="FormValidation">
                <div class="card-body">
                    <div class="row">
                        <div class="col-12">
                            <label for="allowance-name-add" class="form-label"
...
export default {
    ...
    methods: {
        async getData(page = 1) {
            ...
            try {
                const response = await ApiData.get(
                    `app/master/allowances?limit=${this.limit}&page=${this.page}&name=${this.filter.name}`
                );
        ...
        async getDesignation(query) {
            try {
                const response = await ApiData.get(
                    `app/master/designations?name=${query}&department=${this.allowance.department?.id}`
                );
                var data = response.data;
        ...
        removeData(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    NProgress.start();
                    NProgress.set(0.1);
                    ApiData.delete("app/master/allowances/delete/" + id)
                        .then((response) => {
        ...
        createData() {
            ApiData.post("app/master/allowances/create", this.allowance)
                .then((response) => {
                    this.$handleSuccessResponse(response.data.message);
                    NProgress.done();
        ...
        updateData() {
            ApiData.post(
                "app/master/allowances/update/" + this.allowance.id,
                this.allowance
            )
                .then((response) => {
                    this.$handleSuccessResponse(response.data.message);
