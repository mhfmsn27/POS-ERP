                    ...
                    <div class="row">
                        ...
                        <div class="col-12 mt-4">
                            ...
                            <Field
                                ...
                                v-slot="{ errors }"
                                v-model="taxrate.taxrate"
                                ref="codepajak"
                                name="Persentase Pajak"
                            >
                                <InputNumber
                                    style="width: 100%"
                                    v-model="taxrate.taxrate"
            ...
            <div class="card-body p-3">
                <div class="table-responsive">
                    <DataTable
                        ...
                        dataKey="id"
                    >
                        <Column field="name" header="Nama Pajak"></Column>
                        <Column field="code" header="Kode Pajak"></Column>
                        <Column
                            field="taxrate"
...
export default {
    ...
    methods: {
        async getData(page = 1) {
            ...
            try {
                const response = await ApiData.get(
                    `app/master/taxrates?limit=${this.limit}&page=${this.page}&name=${this.filter.name}`
                );
        ...
        removeData(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    NProgress.start();
                    NProgress.set(0.1);
                    ApiData.delete("app/master/taxrates/delete/" + id)
                        .then((response) => {
        ...
        createData() {
            ApiData.post("app/master/taxrates/create", this.taxrate)
                .then((response) => {
                    this.$handleSuccessResponse(response.data.message);
                    NProgress.done();
        ...
        updateData() {
            ApiData.post(
                "app/master/taxrates/update/" + this.taxrate.id,
                this.taxrate
            )
                .then((response) => {
                    this.$handleSuccessResponse(response.data.message);
