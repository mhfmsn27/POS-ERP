        ...
        <div class="card card-block card-stretch card-height">
            ...
            <Form @submit="ValidationFormData()" ref="FormValidation">
                <div class="card-body">
                    <div class="row">
                        <div class="col-12">
                            <label for="designation-name-add" class="form-label"
                                >Nama Jabatan</label
                            >
                            <Field
                                :rules="{
                        ...
                        <div class="col-12 mt-4">
                            ...
                            <Field
                                ...
                                v-slot="{ errors }"
                                v-model="designation.department"
                                name="Devisi"
                            >
                                <Multiselect
                                    v-model="designation.department"
...
export default {
    ...
    methods: {
        async getData(page = 1) {
            ...
            try {
                const response = await ApiData.get(
                    `app/master/designations?limit=${this.limit}&page=${this.page}&name=${this.filter.name}`
                );
        ...
        removeData(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    NProgress.start();
                    NProgress.set(0.1);
                    ApiData.delete("app/master/designations/delete/" + id)
                        .then((response) => {
        ...
        createData() {
            ApiData.post("app/master/designations/create", this.designation)
                .then((response) => {
                    this.$handleSuccessResponse(response.data.message);
                    NProgress.done();
        ...
        updateData() {
            ApiData.post(
                "app/master/designations/update/" + this.designation.id,
                this.designation
            )
                .then((response) => {
                    this.$handleSuccessResponse(response.data.message);
        ...
        formatForm() {
            ...
            this.editmode = false;
            this.$refs.designationname.reset();
        },
    },
};
</script>
