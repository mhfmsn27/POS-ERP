    ...
    methods: {
        async getProducts(query) {
            ...
            try {
                const response = await ApiData.get(
                    `app/inventory/components/variations?name=${query}&warehouse=${this.transaction.from.id}`
                );
        ...
        async getWarehouse() {
            try {
                ...
                this.transaction.from = this.warehouse;
            } catch (error) {
                console.log(error);
            }
        },
        ...
        changeWarehouse() {
            if (this.transaction.items.length > 0) {
                ...
                this.warehouse = {
                    id: this.transaction.from.id,
                    name: this.transaction.from.name,
                };
                this.products = [];
            }
        },
        ...
        updateAdjustment(index) {
            ...
            if (qtyAdjustment > details.quantity) {
                this.$toast.add({
                    severity: "error",
                    summary: "Peringatan!",
                    detail: "Tidak boleh melebihi stok tercatat",
                    life: 3000,
                });

                this.transaction.items[index].qty = 0;
                details.subtotal = 0;
            } else {
        ...
        createStockOpname() {
            this.$refs.ValidationStockOpname.validate().then((success) => {
                if (!success) {
                    ...
                    NProgress.set(0.1);
                    ApiData.post(
                        "app/transactions/transfer-warehouse/store",
                        this.transaction
                    )
                        .then((response) => {
