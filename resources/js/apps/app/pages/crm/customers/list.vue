<template>
    ...
    <div class="col-12">
        <div class="card card-block card-stretch card-height">
            <div class="card-header d-flex justify-content-between p-3">
                <div>
                    <label class="control-label">Nama Customer / Email</label>
                ...
                <div class="d-flex justify-content-start">
                    ...
                    <a
                        ...
                        class="btn btn-info me-2"
                    >
                        <i class="fe fe-plus-circle"></i> Tambah Pelanggan
                    </a>
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    ...
                    >
                        ...
                        <Column header="Default Pelanggan">
                            <template #body="{ data }">
                                <InputSwitch
                                    v-model="data.default"
                                    @input="setDefault(data.id)"
                                />
                            </template>
                        </Column>
                        <Column header="Total Piutang">
                            <template #body="{ data }">
    ...
    <Dialog v-model:visible="modal.import" header="" :style="{ width: '60vh' }">
        <div class="card-body ps-5 pe-5 pt-2 pb-5 rectangle3">
            <p class="h4 fw-semibold mb-2 text-center">Import Data Pelanggan</p>
...
export default {
    ...
    methods: {
        ...
        importData() {
            ...
            ApiData.post("app/crm/customers/import", this.data)
                .then((response) => {
        ...
        async downloadExample() {
            ...
            try {
                const response = await ApiData.get(
                    `app/crm/customers/import/download-sample`,
                    {
                        responseType: "blob",
                        headers: {
                ...
                link.setAttribute("download", "sample_import_customer.xlsx");
        ...
        async getData(page = 1) {
            ...
            try {
                const response = await ApiData.get(
                    `app/crm/customers?limit=${this.limit}&page=${this.page}&name=${this.filter.name}`
                );
        ...
        async setDefault(id) {
            try {
                const response = await ApiData.post(
                    `app/crm/customers/set-default/${id}`
                );
        ...
        removeData(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    NProgress.start();
                    NProgress.set(0.1);
                    ApiData.delete("app/crm/customers/delete/" + id)
                        .then((response) => {
                            this.$handleSuccessResponse(response.data.message);
                            NProgress.done();
                            this.getData();
        ...
        async exportDataSpt() {
            if (!this.loader.submit) {
                ...
                try {
                    const response = await ApiData.get(
                        `app/crm/customers/download-spt?name=${this.filter.name}`,
                        {
                            responseType: "blob",
                            headers: {
                                "Content-Type": "application/json",
                    ...
                    link.setAttribute("download", "customer_spt_format.xlsx");
                    document.body.appendChild(link);
                    link.click();
