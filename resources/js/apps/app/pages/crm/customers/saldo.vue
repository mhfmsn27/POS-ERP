...
export default {
    ...
    methods: {
        ...
        importData() {
            ...
            ApiData.post(
                "app/crm/customers/import/saldo/" + this.$route.params.id,
                this.data
            )
                .then((response) => {
        ...
        async downloadExample() {
            ...
            try {
                const response = await ApiData.get(
                    `app/crm/customers/import/download-saldo`,
                    {
                        responseType: "blob",
                        headers: {
                ...
                link.setAttribute(
                    "download",
                    "sample_export_saldo_customer.xlsx"
                );
        ...
        async getData(page = 1) {
            ...
            try {
                const response = await ApiData.get(
                    `app/transactions/transaction-due?limit=${this.limit}&page=${this.page}&name=${this.filter.name}&customer=${this.$route.params.id}&type=saldo`
                );
