...
export default {
    ...
    methods: {
        ...
        importData() {
            ...
            ApiData.post(
                "app/crm/customers/import/due/" + this.$route.params.id,
                this.data
            )
                .then((response) => {
        ...
        async downloadExample() {
            ...
            try {
                const response = await ApiData.get(
                    `app/crm/customers/import/download-due`,
                    {
                        responseType: "blob",
                        headers: {
                ...
                link.setAttribute(
                    "download",
                    "sample_export_due_customer.xlsx"
                );
        ...
        async getData(page = 1) {
            ...
            try {
                const response = await ApiData.get(
                    `app/transactions/transaction-due?limit=${this.limit}&page=${this.page}&name=${this.filter.name}&customer=${this.$route.params.id}&type=hutang`
                );
        ...
        async historyPayment(id) {
            try {
                const response = await ApiData.get(
                    `app/transactions/transaction-due/history?limit=100&page=1&due=${id}`
                );
                var data = response.data;
        ...
        removeData(data) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    NProgress.start();
                    NProgress.set(0.1);
                    ApiData.delete("app/crm/customers/due/delete/" + data.id)
                        .then((response) => {
                            this.$handleSuccessResponse(response.data.message);
                            NProgress.done();
                            this.getData();
