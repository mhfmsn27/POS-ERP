...
export default {
    ...
    methods: {
        ...
        async getData(page = 1) {
            ...
            try {
                const response = await ApiData.get(
                    `app/ecommerce/orders?limit=${this.limit}&page=${this.page}&ref=${this.filter.name}&start_date=${startdate}&end_date=${enddate}&createdby=${this.filter.user}&customer=${this.filter.customer}&status=transit&sort=${this.sort.field}&sortby=${this.sort.order}`
                );
                var data = response.data;
