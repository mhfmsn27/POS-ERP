<template>
    ...
    <div class="col-lg-9 col-sm-12">
        <div class="card card-block card-stretch card-height">
            ...
            <Form
                @submit="ValidationSliders()"
                ref="SliderValidation"
                class="card-body p-4"
            >
                <div class="row">
                    <!-- Title -->
                    <div class="col-12 mt-4">
                        <label for="category-product">Title Slider</label>
                        <Field
                            :rules="{
                                required: true,
                            }"
                    ...
                    <!-- Title -->
                    <div class="col-12 mt-4">
                        <label for="category-product">Subtitle Slider</label>
                        <InputText
                            v-model="slider.subtitle"
                            style="width: 100%"
                            type="text"
                            class="form-control"
                            placeholder="Masukkan Sub Title Slider"
                        />
                    </div>
                    <!-- End Title -->
...
export default {
    ...
    methods: {
        async getData() {
            try {
                const response = await ApiData.get(
                    `app/ecommerce/media-content/sliders/detail/${this.$route.params.id}`
                );
        ...
        ValidationSliders() {
            this.$refs.SliderValidation.validate().then((success) => {
                if (!success) {
                    this.$toast.add({
                        severity: "error",
        ...
        createData() {
            ApiData.post(
                `app/ecommerce/media-content/sliders/update/${this.$route.params.id}`,
                this.slider
            )
                .then((response) => {
