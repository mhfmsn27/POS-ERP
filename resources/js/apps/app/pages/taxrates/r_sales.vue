        ...
        async getData(page = 1) {
            ...
            try {
                const response = await ApiData.get(
                    `app/taxs?limit=${this.limit}&page=${this.page}&name=${this.filter.name}&start_date=${this.filter.date.start}&end_date=${this.filter.date.end}&sub_type=tax_output_return&status_payment=${this.filter.payment_status}&status=${this.filter.status}`
                );
