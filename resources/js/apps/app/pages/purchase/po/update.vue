...
export default {
    ...
    methods: {
        ...
        RemoveItem(id, index) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    NProgress.set(0.1);
                    ApiData.delete(
                        "app/transactions/purchases/po/delete-item/" + id
                    )
                        .then((response) => {
        ...
        savingItem(item) {
            ApiData.post(
                "app/transactions/purchases/po/update-item/" + item.id,
                item
            )
                .then((response) => {
                    this.$handleSuccessResponse(response.data.message);
        ...
        createpoPurchase(type = "") {
            this.$refs.ValidationTransactions.validate().then((success) => {
                if (!success) {
                    ...
                    NProgress.set(0.1);
                    ApiData.post(
                        "app/transactions/purchases/po/update/" +
