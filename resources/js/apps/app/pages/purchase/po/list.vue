            ...
            <div class="card-body p-0 table-responsive">
                ...
                >
                    ...
                    <Column header="Status" v-if="isColumnVisible('status')">
                        <template #body="{ data }"> 
                            <span class="badge bg-warning text-black" v-if="data.status == 'open'">Buka</span>
                            <span class="badge rounded-pill bg-info" v-if="data.status == 'close'">Tutup</span>
                        </template>
                    </Column>
...
export default {
    ...
    methods: {
        async getOptions() {
            try {
                const response = await ApiData.get(
                    `app/settings/table-view?table=purchase_po`
                );
        ...
        async getData(page = 1) {
            ...
            try {
                const response = await ApiData.get(
                    `app/transactions/purchases/po?limit=${this.limit}&page=${this.page}&ref=${this.filter.name}&start_date=${startdate}&end_date=${enddate}&createdby=${this.filter.user}&supplier=${this.filter.supplier}&sort=${this.sort.field}&sortby=${this.sort.order}&status=${this.filter.status}`
                );
                var data = response.data;
        ...
        deleteData(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    NProgress.set(0.1);
                    ApiData.delete(
                        "app/transactions/purchases/po/delete/" + id
                    )
                        .then((response) => {
