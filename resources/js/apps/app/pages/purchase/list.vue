        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                ...
                <div class="d-flex justify-content-start">
                    ...
                    >
                        <i class="fa fa-plus-circle me-2"></i> Buat Pembelian
                    </a>
                    <button
                        type="button"
...
export default {
    ...
    methods: {
        async getOptions() {
            try {
                const response = await ApiData.get(
                    `app/settings/table-view?table=purchase_faktur`
                );
        ...
        async getData(page = 1) {
            ...
            try {
                const response = await ApiData.get(
                    `app/transactions/purchases?limit=${this.limit}&page=${this.page}&ref=${this.filter.name}&start_date=${startdate}&end_date=${enddate}&createdby=${this.filter.user}&supplier=${this.filter.supplier}&payment=${this.filter.status}&sort=${this.sort.field}&sortby=${this.sort.order}`
                );
                var data = response.data;
        ...
        deleteDraft(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    NProgress.set(0.1);
                    ApiData.delete(
                        "app/transactions/purchases/delete-draft/" + id
                    )
                        .then((response) => {
