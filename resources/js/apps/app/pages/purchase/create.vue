...
export default {
    ...
    methods: {
        ...
        async handleForPurchaseReceived(id) {
            try {
                const response = await ApiData.get(
                    `app/transactions/purchases/received/detail/${id}`
                );
                var data = response.data.details;
                this.transaction.general_information.supplier = data.supplier;
                this.changeSupplier(data.supplier);
        ...
        updateTransaction() {
            ...
            this.transaction.payment_information.pay_total = afterShipping;
        },
            ...
            this.$refs.ValidationForOtherInformation.validate().then(
                (success) => {
                    if (!success.valid) {
                        ...
                        NProgress.set(0.1);
                        ApiData.post(
                            "app/transactions/purchases/create",
                            this.transaction
                        )
                            .then((response) => {
...
<body><div class="container">
    ...
    mounted: function () {
        ...
        if (this.$route.query.purchase_received != undefined) {
            this.handleForPurchaseReceived(this.$route.query.purchase_received);
        }
    },
    watch: {
        "transaction.items": {
