    ...
    >
        <div class="row">
            ...
            <div class="col-1">
                ...
                <button
                    type="button"
                    v-tooltip="'Proses ke Faktur Pembelian'"
                    @click="
                        $goTo({
                            name: 'purchase_create',
                            query: {
                                purchase_received: this.$route.params.id,
                            },
                        })
                    "
                    class="btn btn-success btn-block label-btn label-end"
                >
                    <i
                        class="fa fa-money label-btn-icon"
                        style="font-size: 30px"
                    ></i>
                </button>
            </div>
...
export default {
    ...
    methods: {
        ...
        async getDetail() {
            try {
                const response = await ApiData.get(
                    `app/transactions/purchases/received/detail/${this.$route.params.id}`
                );
                var data = response.data;
        ...
        RemoveItem(id, index) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    NProgress.set(0.1);
                    ApiData.delete(
                        "app/transactions/purchases/received/delete-item/" + id
                    )
                        .then((response) => {
        ...
        savingItem(item) {
            ApiData.post(
                "app/transactions/purchases/received/update-item/" + item.id,
                item
            )
                .then((response) => {
                    this.$handleSuccessResponse(response.data.message);
        ...
        createReceivedPurchase(type = "") {
            this.$refs.ValidationTransactions.validate().then((success) => {
                if (!success) {
                    ...
                    NProgress.set(0.1);
                    ApiData.post(
                        "app/transactions/purchases/received/update/" +
