...
class SalesPaymentObserver
{
    ...
    public function __construct(TransactionDueObserver $transactionDueObserver, TransactionPaymentObserver $transactionPaymentObserver, LedgerTransactionObserver $ledgerTransactionObserver, LedgerObserver $ledgerObserver, CommissionObserver $commissionObserver)
    {
    ...
    public function createOrUpdateTransaction(Request $request, Transaction $transaction)
    {
            ...
            if ($allocatedQty > 0) {
                ...
                if ($dueDetail->transaction) {
                    ...
                    if ($dueDetail->total_due < 1) {
                        $this->commissionObserver->createData($dueDetail->transaction);
                    }
                }
        ...
        foreach ($collection->where("type", "saldo") as $faktur) {
            ...
            if ($allocatedQty < 0) {
                ...
                if ($dueDetail->transaction) {
                    if ($dueDetail->transaction->type == 'sales_return') {
                        $dueDetail->transaction->transaction->update([
                            'payment_status'        => $dueDetail->transaction->due_total < 1 ? 'paid' : 'due'
                        ]);

                        if ($dueDetail->transaction->due_total_return_sell < 1 && $dueDetail->transaction->transaction->commission) {
                            $this->commissionObserver->updateData($dueDetail->transaction->transaction, $dueDetail->transaction->transaction->commission);
                        }

                        $dueDetail->transaction->update([
                            'payment_status'        => $dueDetail->transaction->due_total_return_sell < 1 ? 'paid' : 'due'
                        ]);
                    }
                }
    ...
    public function deleteItem(FakturPaymentDetail $faktur, Transaction $transaction)
    {
        ...
        if ($faktur->transaction_due->transaction) {
            $faktur->transaction_due->transaction->update([
                'payment_status'        => abs($faktur->transaction_due->transaction->due_total) < 1 ? 'paid' : 'due'
            ]);

            if ($faktur->transaction_due->transaction->commission && abs($faktur->transaction_due->transaction->due_total) > 1) {
                $faktur->transaction_due->transaction->commission->delete();
            }


            if ($faktur->transaction_due->transaction->type == 'sales_return') {
                $faktur->transaction_due->transaction->transaction->update([
                    'payment_status'        => abs($faktur->transaction_due->transaction->transaction->due_total) < 1 ? 'paid' : 'due'
                ]);
            }
        }
