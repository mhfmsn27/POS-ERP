...
class CheckOutController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $settings         = EcommerceApiSetting::where('store_id',session()->get('dfstore'))->first(['client_key', 'price_per_km', 'kurir_manual']);
        $tax_total        = 0;
        $grandTotal       = $carts['subtotal'];
        $address          = $this->addressRepository->getData();
        $dAddress         = $this->addressRepository->defaultAddress();
        ...
        return view('ecommerce::mobile.account.checkout', ['page' => 'Checkout Pesanan'], compact('carts', 'stores', 'tax_total', 'grandTotal', 'address', 'settings', 'dAddress'));
    }
